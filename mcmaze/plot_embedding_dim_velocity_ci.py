...
results = {
    ...
    "Fine-tuned": {
        ...
        "Output": {
            ...
            "R2": np.array([
                [0.9101, 0.9094, 0.9092, 0.9092, 0.9091],
...
# -----------------------
frozen_color = "#A9D6F5"      # light icy blue
finetuned_color = "#7BC47F"   # greenish
...
def plot_metric(metric):
    ...
    for model in ["Frozen", "Fine-tuned"]:
        for layer in ["Hidden", "Output"]:
            mean, ci95 = mean_and_ci95(results[model][layer][metric])
            style = style_map[(model, layer)]
    ...
    if metric == "RMSE":
        plt.ylabel("RMSE")
        plt.title("Hand velocity prediction RMSE vs. embedding dimension")
        base_name = "rmse_vs_embedding_dimension_all"
    else:
        plt.ylabel(r"$R^2$")
        plt.title(r"Hand velocity prediction $R^2$ vs. embedding dimension")
        base_name = "r2_vs_embedding_dimension_all"

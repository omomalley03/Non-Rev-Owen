...
def compute_hidden_features(model, dataset, batch_size: int, device: torch.device):
    ...
    if len(model.net) <= 1 or not isinstance(model.net[-1], torch.nn.Linear):
        raise ValueError("Cannot remove final linear layer: model.net does not end with a hidden-to-output Linear")

"""
Fine-tuning sweep for block CCA regularizer (sum-S objective).

Based on v1 results: the sweet spot is lambda_block_cca in [2, 5].
This sweep fills in that range with finer granularity and tests a few
CCA + small plane_bt combinations.

After all runs complete, generates CCA diagnostic figures for the top 5.

Usage
-----
    python sweep_block_cca_v2.py --dry-run
    python sweep_block_cca_v2.py
    python sweep_block_cca_v2.py --epochs 100
"""
...
def make_experiment_grid(base_cfg: Config) -> list[tuple[str, Config]]:
    ...
    cfg = make_base(base_cfg)
    cfg.lambda_bt = 0.12
    add("baseline_bt_0.12", cfg)

    # ── Fine-grained CCA sweep: fill in [1.5, 8] ─────────────────────
    for lam in [1.5, 2.0, 2.5, 3.0, 3.5, 4.0, 5.0, 6.0, 8.0]:
        cfg = make_base(base_cfg)
        cfg.lambda_block_cca = lam
        add(f"cca_{lam:g}", cfg)

    # ── CCA + small plane_bt (best combos from v1) ────────────────────
    for lam_cca in [2.0, 3.0, 4.0, 5.0]:
...
def main() -> None:
    ...
    if args.dry_run:
        print(f"Block CCA v2 sweep: {len(experiments)} experiments, {args.epochs} epochs\n")
        print(f"{'#':>3}  {'experiment':<30} {'cca':>5} {'pbt':>6} {'bt':>5}")
        print("-" * 55)
        for idx, (name, cfg) in enumerate(experiments, start=1):
            print(
                f"{idx:>3}  {name:<30} "
                f"{cfg.lambda_block_cca:>5g} {cfg.lambda_plane_bt:>6g} {cfg.lambda_bt:>5g}"
            )
        return

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    sweep_dir = args.sweep_dir or os.path.join("runs", f"block_cca_v2_{timestamp}")
    ...
    if args.top_vis > 0:
        ...
        for i, row in enumerate(top_n, start=1):
            run_dir = row["run_dir"]
            out_path = os.path.join(sweep_dir, f"top{i}_{row['experiment']}_cca.png")
            try:

...
Usage:
    micromamba run -n nonrev python -u sweep_scripts/physionet/run_physionet_embedding_dim_condition_decoder_sweep.py
"""
...
def run_jobs(args: argparse.Namespace) -> None:
    ...
    for idx, (dim, seed, run_dir) in enumerate(jobs, 1):
        ...
        cmd = [
            python_exe,
            "-u",
            str(REPO_ROOT / "decoders/predict_physionet_condition_finetune.py"),
            "--run",
            str(run_dir),
            "--epochs",
            str(args.epochs),
